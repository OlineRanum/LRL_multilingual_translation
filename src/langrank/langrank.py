...
POS_DATASETS = {"ud": "ud.npy"}
EL_DATASETS = {"wiki": "wiki.npy"}
DEP_DATASETS = {"conll": "conll.npy"}
...
POS_MODELS = {"best": "lgbm_model_pos_all.txt"}
...
def check_task(task):
    if task not in TASKS:
        raise Exception(
            "Unknown task " + task + ". Only 'MT', 'DEP', 'EL', 'POS' are supported."
        )
...
def get_candidates(task, languages=None):
    ...
    for dt in datasets_dict:
        fn = pkg_resources.resource_filename(
            __name__, os.path.join("indexed", task, datasets_dict[dt])
        )
    ...
    if languages is not None and task == "MT":
        ...
        if len(add_languages) > 0:
            new_cands = [
                c
                for c in cands
                if c[0][-3:] in add_languages and c[0][-3] not in sub_languages
            ]
        else:
...
def prepare_new_dataset(
    ...
    if (
        dataset_source is None
        and dataset_target is None
        and dataset_subword_source is None
        and dataset_subword_target is None
    ):
        print(
            "NOTE: no dataset provided. You can still use the ranker using language typological features."
        )
        return features
    elif dataset_source is None:  # and dataset_target is None:
        print(
            "NOTE: no word-level dataset provided, will only extract subword-level features."
        )
    ...
    if task == "MT":
        ...
        if isinstance(dataset_subword_source, str):
            ...
            raise Exception(
                "dataset_subword_source should either be a filnename (str) or a list of sentences."
            )
        if source_lines:
            features["dataset_size"] = len(
                source_lines
            )  # This should be be the same as above
            tokens = [w for s in source_lines for w in s.strip().split()]
            features["subword_token_number"] = len(tokens)
            types = set(tokens)
            features["subword_type_number"] = len(types)
            features["subword_vocab"] = types
            features["subword_type_token_ratio"] = features[
                "subword_type_number"
            ] / float(features["subword_token_number"])
...
def distance_vec(test, transfer, uriel_features, task):
    ...
    if task != "EL":
        word_overlap = float(
            len(set(transfer["word_vocab"]).intersection(set(test["word_vocab"])))
        ) / (transfer["type_number"] + test["type_number"])
    elif task == "EL":
        word_overlap = float(
            len(set(transfer["word_vocab"]).intersection(set(test["word_vocab"])))
        )
    # Subword overlap
    if task == "MT":
        subword_overlap = float(
            len(set(transfer["subword_vocab"]).intersection(set(test["subword_vocab"])))
        ) / (transfer["subword_type_number"] + test["subword_type_number"])

    if task == "MT":
        data_specific_features = [
            word_overlap,
            subword_overlap,
            transfer_dataset_size,
            task_data_size,
            ratio_dataset_size,
            transfer_ttr,
            task_ttr,
...
def rank(
    test_dataset_features, task="MT", candidates="all", model="best", print_topK=3
):
    """
    ...
    # load model
    print("Loading model...")
    model_dict = map_task_to_models(
        task
    )  # this loads the dict that will give us the name of the pretrained model
    model_fname = model_dict[
        model
    ]  # this gives us the filename (needs to be joined, see below)
    modelfilename = pkg_resources.resource_filename(
        __name__, os.path.join("pretrained", task, model_fname)
    )
    ...
    for j in range(len(feature_name)):
        if sort_sign_list[j] != 0:
            ...
            for i in range(TOP_K):
                ...
                print(
                    "%d. %s : score=%.2f"
                    % (i, candidate_list[index][0], test_inputs[index][j])
                )
    ...
    for j, i in enumerate(ind[:print_topK]):
        ...
        print(
            "\t1. %s : score=%.2f; \n\t2. %s : score=%.2f; \n\t3. %s : score=%.2f"
            % (
                feature_name[contrib_ind[0]],
                contrib_scores[contrib_ind[0]],
                feature_name[contrib_ind[1]],
                contrib_scores[contrib_ind[1]],
                feature_name[contrib_ind[2]],
                contrib_scores[contrib_ind[2]],
            )
        )
    return ind
